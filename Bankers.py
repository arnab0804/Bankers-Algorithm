# AIM: To implement the Banker's Alogorithm
# Date: 5/6/2023
...
# Read allocation matrix and total vectors.
alloc=[]
for i in range(n):
    a=[]
    for j in range(m):
...
def bankers(maxim, alloc, total, n, m):
    ...
    safe = [-1]*n
    need = []
    # Calculating the need of the resources
    for i in range(n):        
        a =[]
        for j in range(m):    
    ...
    while (True):
        ...
        if (status == -2):
            ...
            # current process can still be explored
            flag=0
            for j in range(m):
